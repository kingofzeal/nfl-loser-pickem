...
- [x] **Option A:** A team that **won** in that specific week (requires waiting until all games are final)
...
- [a] **Option A:** Treat ties as losses (current spec)
...
- [x] **Option D:** Hybrid (auto with admin override option)
...
- [x] **Option A:** Workspace-wide only (all players get same reminders)
...
- [x] **Option A:** One league only (simplest)
...
- [x] **Option C:** Export to JSON/CSV and purge
...
- [x] **Option B:** After week finalization (show which team was picked in summary)
...
- [x] **Option A:** Not supported (18-week regular season only)
...
- [x] **Option C:** Yes - they only participate in remaining weeks (no penalties)
...
- [x] **Option B:** Unlock picks and extend deadline to new kickoff time
...
- [x] **Option A:** Everyone who picked gets a loss
...
- [x] **Option C:** Admin must fix

**Your Decision:**
Players should not be allowed to pick teams that do not play that week.
---

## 12. Platform Priority
**Question:** Which platform should be developed first:
- [x] **Slack** (more common in workplaces)
...
- [x] **Option A:** Server-side with canvas library
...
- [x] **Option A:** By platform user ID only (user must send command from same account)
...
- [x] **Moderate:** Above + send DMs, post to specific channels
